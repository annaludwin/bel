...
let BeL = document.body.innerHTML;
const html = localStorage.getItem("BeL")
if(html !== null) {
  document.body.innerHTML = JSON.parse(html)
}
...
function looser(){
  ...
  const clearTable = document.getElementById("bigMac")
  ...
  // czyszczenie tablicy
  clearTable.addEventListener("click", clearSchedule)
  ...
  function doShoppingList(e) {
    if (e.target.className === "shop") {
      ...
      // update LS
      localStorage.setItem("BeL", JSON.stringify(document.body.innerHTML))
    }
  }
